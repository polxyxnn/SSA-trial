...
# ====================== SAFE PATHS (EXACTLY FOR YOUR STRUCTURE) ======================
# Your folder layout: SSA-tools/rocket.py  +  SSA-tools/utils/shapefiles/
if "shape_dir" not in st.session_state:
    current_file_dir = os.path.dirname(os.path.abspath(__file__))          # → SSA-tools
    st.session_state.shape_dir = os.path.join(current_file_dir, "utils", "shapefiles")
    
    # Remove this line after you confirm the path is correct
...
def extract_notam_data(uploaded_file):
    ...
    try:
        ...
        c_time = c_match.group(1)[-4:] if c_match and len(c_match.group(1)) >= 10 else ""   # ← FIXED
        if not c_time:
...
def create_folium_map(launch_site_value, dropzones, shape_dir):
    ...
    # ====================== BOUNDARY LOADING (clear messages) ======================
